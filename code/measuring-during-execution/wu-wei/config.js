...
var os = require('os')
var cpuCollector = require('./../lib/cpu-collector.js')
function deepcopy (o) {
...
function getConfigFromFileSystem (dirRoot, cb) {
    ...
    dir.files(dirRoot, function (err, files) {
        ...
        var suiteRoot = exports.findRootDirectoryPath(process.cwd(), '.wu')
        var wuDependencyCache = path.join(suiteRoot, '.wu', 'dependencies')
...
exports.executeRun = function (config, options) {
    return new Promise((resolve,reject)=>{
        ...
        var environmentFlags = (config.environment['environment-flags'])?config.environment['environment-flags'].join(' '):''
        var cmd = environmentRunPath + ' ' + runnerPath + ' ' + runnerArguments + ' ' +environmentFlags
        //Start CPU runner process.
        var startCpu = cpuCollector.totalCpuTimeTicks()
        const child_cpu = shelljs.exec('node '+path.join(__dirname,'./cpu-collector.js --run'), {async: true, silent: true});
        const child_runner = shelljs.exec(cmd, {async: true, silent: !options.verbose});
        var status = null;
        var totalCpuValues = [];
        var meanCpu = null;
        ...
        child_cpu.stdout.on('data', function (data) {
            let val = data.split("\n");
            val = Number(val[0]);
            totalCpuValues.push(val);
        });
        
        child_cpu.on('close',()=>
        {
            var endCpu = cpuCollector.totalCpuTimeTicks()
            var startEndMean = cpuCollector.computeMeanCpu(startCpu,endCpu)
            totalCpuValues.push(startEndMean);
            meanCpu = mathjs.mean(totalCpuValues).toFixed(2);
            if(options.verbose) console.log("Mean CPU:" + meanCpu+", Total Number of Measurements: "+ totalCpuValues.length);
            var jsonOutputMatch = status.match(/\{(.*\n?)+\}/)
            var jsonOutput
            ...
            if (!options['skip-output-verification'] &&
                ...
                var expectedOutput = config.benchmark['expected-output'][config.experiment['input-size']]
            ...
            resolve({output: jsonOutput, cpuAvgLoad:meanCpu,cpuValues:totalCpuValues});
        });
