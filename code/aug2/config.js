...
exports.executeRun = function (config, options,iteration) {
  ...
  fs.appendFileSync(path.join(__dirname,"../",`bench=${config.benchmark["short-name"]}-impl=${config.implementation["short-name"]}-comp=${config.compiler['short-name']}-env=${config.environment['short-name']}.csv`),`\niter=${iteration}: `);
...
function computeAverageCpu(start,end)
{
